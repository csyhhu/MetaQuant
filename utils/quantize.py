...
from utils.miscellaneous import progress_bar, accuracy, AverageMeter
...
class quantized_CNN(nn.Conv2d):
    ...
    def forward(self, input, quantized):
        ...
        if quantized == 'dorefa':
            ...
            # self.pre_quantized_weight = self.weight / self.alpha.data # [-inf, inf]
            ...
            # self.pre_quantized_weight = self.weight / self.alpha
            self.pre_quantized_weight = self.weight
            self.quantized_weight = self.alpha.data * Function_BWN.apply(self.pre_quantized_weight)
        else:
